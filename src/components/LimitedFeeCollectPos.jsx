import { Box, IconButton, Select, Input, Text, HStack, Button } from '@chakra-ui/react'
import React from 'react'
import {BsArrowReturnLeft, BsQuestionCircle} from 'react-icons/bs'

export default function LimitedFeeCollectPos({
  swithFromLimitedFeeToPage, 
  selectedCurrency, setselectedCurrency,  postPrice,  setpostPrice,
  referalFee, setreferalFee , postPermission, setpostPermission, 
  setcollectLimit,  collectLimit
}) {
  return (
    <Box>
   <IconButton   icon={<BsArrowReturnLeft color='black' size={22} onClick={swithFromLimitedFeeToPage}/>}     />
   <Box mt={3}>
        ...
        <Text mt={4}>Collect Limit</Text>
        <Input      type="number"  placeholder='5' value={collectLimit} onChange = {e => setcollectLimit(e.target.value)} />
       ...
       <Text mt={3}>Permision</Text>
        ...
        <Button w="100%" mt={6} color="black" onClick={swithFromLimitedFeeToPage}>save</Button>
